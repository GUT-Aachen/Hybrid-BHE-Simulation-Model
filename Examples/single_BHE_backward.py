...
def main():
	...
	BheData = 		{	    'id'    : 'BHE01',
							...
							'capG':   1000000,		# [J/m³/K]	volumetric heat capacity grout
							...
							'xCoord' : 3	,		# [m]		x coordinate
							'yCoord' : 4	,		# [m]		y coordinate
							'Qf' : 1.92/3600.,		# [m³/s]	flow rate		
							'Tundist': 12.0,		# [°C]		undisturbed ground temperature
							'lm' : 2.3,				# [W/m/K]	thermal conductivity ground	
							'Cm' : 2300000.,		# [J/m³/K]	volumetric heat capacity ground
	}

	
	sim_setup = {	'nz': 	100,			# bhe cells vertical
				'dt_bhe': 	5,				# bhe time step
				'dt_soil': 	5,				# soil time step
				'nt_part':	17280,			# soil time steps per period
	...
	M_Tin = np.ones(sim_setup['nt_part']*sim_setup['n_parts'])*30
	...
	res_loads[:,0:sim_setup['nt_part']]) = hybrid_model.calc_sa_sec_U_backw(sim_setup,
																  gMatrix,
	...
	for p in range(1,sim_setup['n_parts']):
		...
		res_loads[:,p*sim_setup['nt_part']:(p+1)*sim_setup['nt_part']]) = hybrid_model.calc_sa_sec_U_backw(sim_setup,
																								 gMatrix,
	...
	plt.plot(BHEs[0].Tf_in)
	plt.plot(BHEs[0].Tf_out)
	plt.show()
